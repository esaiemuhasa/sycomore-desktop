...
public class ChartTestFrame extends JFrame {
	...
	public ChartTestFrame() throws HeadlessException {
		...
		listCos.get(0).setType(PointCloud.CloudType.STICK_CHART);
		listSin.get(0).setType(PointCloud.CloudType.STICK_CHART);
...
//		custom.setType(CloudType.STICK_CHART);
		...
		custom4.setType(PointCloud.CloudType.STICK_CHART);
		...
		for (double i = 0; i <= 1.1; i += 0.1) {
			...
			DefaultPiePart part = new DefaultPiePart(Config.getColorAt(index), val, "Titre du part "+index);
			pie.addPart(part);
			index++;
		}
		
		pie.setRealMaxPriority(true);
		PiePanel piePanel = new PiePanel(pie);
