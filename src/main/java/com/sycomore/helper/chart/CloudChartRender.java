...
public class CloudChartRender extends JComponent implements Printable {
	...
	//render data
	private final List<PreparedPointCloud> chartMetadata = new ArrayList<>();
	...
	private final float paddingBottom = 30f;
	...
	//grid
	private final BasicStroke gridStroke = new BasicStroke(0.8f);
	...
	private final int [][] fleshXLineAxis = new int [2][3];
	private final int [][] fleshYLineAxis = new int [2][3];
	private final Rectangle2D borderRect = new Rectangle2D.Double();//rectangle de la bordure
	//
	...
	private final BasicStroke mouseLineWidth = new BasicStroke(1.2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
	...
	protected void paintComponent(Graphics g) {
		...
		if (mouseLocation != null ) {
			...
			if (hoverChart != -1 && hoverPoint != -1) {
				PreparedPointCloud c = chartMetadata.get(hoverChart);
				PreparedMaterialPoint p = chartMetadata.get(hoverChart).getPoints()[hoverPoint];
	...
	private void doPaint(Graphics2D g2, double width, double height) {
		for (int i = 0, count = chartMetadata.size(); i < count; i++) {
			PreparedPointCloud  cloud = chartMetadata.get(i);
			...
			g2.setStroke(mouseLineWidth);
			if (chart.getType() == PointCloud.CloudType.STICK_CHART) {
			...
			if (cloud.getCloud().getType() == PointCloud.CloudType.STICK_CHART)
				continue;
	...
	private void paintGrid (Graphics2D g, double width, double height) {
		...
		if(xLineAxis != null) {//pour le cas où l'axe est visible
			...
			for (double i = xLineAxis.getY1(); y <= height; i += gridYStep) {
				...
				g.setColor(lineAxisColor);
				g.drawString(txt, 0f, y + metrics.getHeight() / 4f);
			}
	...
	protected class MouseListener extends MouseAdapter {
		...
		public void mouseMoved (MouseEvent e) {
			if (borderRect.contains(e.getPoint()) && model.getXMax() != null) {				
				...
				for (int i = 0; i < chartMetadata.size(); i++) {
					PreparedPointCloud c = chartMetadata.get(i);
	 ...
	 * @author Esaie MUHASA
	 */
	public interface ChartRenderTranslationListener {
